    ...
    productsId.innerHTML += `
            <div class="col-lg-3 mb-3">
                <div class="product">
                    ...
                    <button class="product__basket-btn" data-things=1 data-photo=${product.imgUrl} data-id=${product.id} data-name=${product.name} data-price=${product.price}>Добавить в корзину</button>
                    <button class="product__basket-btn-delete" data-things=1 data-photo=${product.imgUrl} data-id=${product.id} data-name=${product.name} data-price=${+product.price}>Удалить из корзины</button>
                </div
            </div>
`
})
...
function deleteToBasket(e) {
    ...
    if (cart[data.id]) {
        ...
        alert('данного товара и так нет в корзине')
    }
